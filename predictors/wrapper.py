...
def iterate_random_values(S, n_check):
	...
	:param S: The dictionary of items
	:param n_check: Size of combination
...
class Splitter():
	...
	:param horizon: Window size - determines how many previous symbols are considered by the model when predicting
	:param n_splits: The number of splits for cross-validation process
	"""

	def __init__(self, trajectories_frame, test_ratio, horizon, n_splits):
		"""
	...
	def _test_split(self, X, Y):
		...
		:param X: Windowed features
		:param Y: Windowed targets
		:return: Training and testing sets of data
		"""
	...
	def _cv_split(self, frame_X, frame_Y, n_splits=5):
		...
		:param frame_X: Training features
		:param frame_Y: Training targets
		:param n_splits: The number of splits to be applied
		"""
	...
	def _stride_data_single(self, frame):
		...
		:param frame: TrajectoriesFrame of single user
		:return: Chunks of data in a DataFrame - features and targets
		"""
		to_concat = []
		for uid, traj in frame.groupby(level=0):
			...
			traj = traj.set_index(['userid', 'datetime'])
...
class SKLearnPred():
	...
	:param algorithm: An algorithm from sklearn library
	:param training_data: The training set from Splitter class
	:param test_data: The test set from Splitter class
	:param param_dist: Hyperparameters dictionary from which the best hyperparameters will be chosen
	:param search_size: The search size for hyperparameters (the number of random combinations to chechk)
	:param cv_size: The number of tests run of every cross validation
	:param parallel: Whether random search should be performed using multithreading
	"""
	...
	def _user_learn(self, args_x, args_y, vals_x, vals_y):
		...
		:param args_x: training features
		:param args_y: training targets
		:param vals_x: validation features
		:param vals_y: validation targets
		:return: user id and score board with accuracy for each hyperparameters combination
		"""
...
class Baseline():
	...
	:param test_data: Test dataset. Do not use split data.
	"""
...
class TopLoc():
	...
	:param train_data: Train dataset. Do not use split data.
	:param test_data: Test dataset. Do not use split data.
	"""
...
def markov_wrapper(trajectories_frame, split_ratio=.8, state_size=1, update=False, averaged=True, online=True):
	"""
	The wrapper, one stop shop algorithm for the Markov Chain. Splits the data, learns the model and makes predictions
	on the test set.
	:param trajectories_frame: TrajectoriesFrame class object
	:param split_ratio: The training set size.
	:param state_size: The order of the Markov Chain
	:param update: Whether the model should update its beliefs based on own predictions
	:param averaged: Whether an averaged accuracy should be returned
	:param online: Whether the algorithm should make an online prediction (sees the last n symbols when predicting)
	:return:
	"""
