...
@jit(nopython=True)
def unq_counts_model(matches, nexts):
    stacked_model = np.column_stack((matches, nexts))

    unique_vals = numba.typed.List()
    unique_counts = numba.typed.List()
    for row in stacked_model:
        found = False
        for i in range(len(unique_vals)):
            if np.all(unique_vals[i] == row):
                unique_counts[i] += 1
                found = True
                break
        if not found:
