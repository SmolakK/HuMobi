...
from src.humobi.misc.utils import get_diags, normalize_chain, _equally_sparse_match, _equally_sparse_match_old, \
    remove_subset_rows, _equally_sparse_match_jit, sparse_predict_jit, unq_counts_model
...
class Sparse(object):
    ...
    def predict(self, context, recency_weights=None, length_weights=None, from_dist=False,
        ...
        if jit:
            prediction = sparse_predict_jit(context, self.model[0], self.model[1], self.model[2], recency_weights, length_weights,
                                            from_dist,
