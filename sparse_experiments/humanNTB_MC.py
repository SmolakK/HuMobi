...
df = TrajectoriesFrame("D:\\Projekty\\bias\\london\\london_1H_111.7572900082951_1.csv",
                       {'names':['id','datetime','temp','lat','lon','labels','start','end','geometry'],"skiprows":1})
...
fname = open(os.path.join(top_path,'humanNTB_MC_times.txt'),'w')
...
for state_size in range(10):
	...
	MC1.to_csv(open(os.path.join(top_path,'humanNTB_MC'+str(state_size)+'.csv'),'w'))
	predictions.to_csv(open(os.path.join(top_path,'predictions_humanNTB_MC'+str(state_size)+'.csv'),'w'))
