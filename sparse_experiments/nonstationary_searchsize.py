...
df = TrajectoriesFrame(os.path.join(top_path,"nonstationary.csv")).loc[:20]
test_size = .2
...
fname = open(os.path.join(top_path,'nonstationary_search_space_times.txt'),'w')
...
with open(os.path.join(top_path,'nonstationary_search_space.json'),'w') as ff:
	json.dump(scores,ff)