...
df = df.uloc(df.get_users()[:20]).fillna(0)
...
fname = open(os.path.join(top_path,'humanNTB_search_space_times.txt'),'w')
fname.write("20 users \n")

scores = {}
for n in [1,5,10,15,20,25,30,35,40,50,55,60]:
	lstart = time()
...
with open(os.path.join(top_path,'humanNTB_search_space.json'),'w') as ff:
	json.dump(scores,ff)