...
df = TrajectoriesFrame(os.path.join(top_path,"nonstationary.csv"))
test_size = .2
...
fname = open(os.path.join(top_path,'nonstationary_MC_times.txt'),'w')
...
for state_size in range(10):
	...
	MC1.to_csv(open(os.path.join(top_path,'nonstationary_MC'+str(state_size)+'.csv'),'w'))
	predictions.to_csv(open(os.path.join(top_path,'predictions_nonstationary_MC'+str(state_size)+'.csv'),'w'))
	print(MC1.mean())
