...
for usr in range(0,len(df.get_users())-2,2):
	...
	rf_scores.to_csv(os.path.join(top_path,'RF_part','%s_markovian_RF_scores.csv' % usr))
	rf_predictions.to_csv(os.path.join(top_path, 'RF_part', '%s_markovian_RF_predictions.csv' % usr))
fname.close()