...
class GridAggregation():
	"""
	A class for spatial aggregation of movement trajectories. This one uses regular grid.
	"""
	...
	def __init__(self, resolution=300, origin=True, x_min=None, x_max=None, y_min=None, y_max=None):
		...
		of data.
		:param resolution: grid resoultion (default = 3000
		:param origin: if the grid should be centered to the centroid of the data
		:param x_min: minimum x value of the grid
		:param x_max: maximum x value of the grid
		:param y_min: minimum y value of the grid
		:param y_max: maximum y value of the grid
		"""
		self._x_min = x_min
		self._x_max = x_max
		self._y_min = y_min
	...
	def _aggregate_multi(self, x, y, centroid_x, centroid_y, row_index, val):
		"""
		Fast movement trajectories spatial aggregation prepared for multithread processing
		:param x: grid x coordinates
		:param y: grid y coordinates
		:param centroid_x: grid x coordinates of centroids
		:param centroid_y: grid y coordinates of centroids
		:param row_index: data row index
		:param val: data row value
		:return: assigned geometry and row index
		"""
	...
	def aggregate(self, trajectories_frame, inplace=True, parralel=False):
		...
		:param inplace: determines if coordinates should be overwritten. If false, a new column is added.
		:param parralel: determines if should uses multithreading (default = False, because it is slower for small data)
		:return:
		"""
...
class ClusteringAggregator():
	...
	def __init__(self, algorithm, **kwargs):
		"""
		Class initialisation. Accepts sklearn clustering algorithms' classes and their kwargs.
		:param algorithm:
		:param kwargs:
		"""
	...
	def _user_aggregate(self, single_trajectory):
		...
		:param single_trajectory: single movement trajectory
		:return: a TrajectoriesFrame with labels column
		"""
		try:
	...
	def _recalcuate_centres(self, single_trajectory):
		...
		:param single_trajectory: single movement trajectory
		:return: a TrajectoriesFrame with overwritten coordinates in lon and lat columns
		"""
	...
	def aggregate(self, trajectories_frame, drop_noise=False, centres_as_geometry=True):
		...
		:param drop_noise: If noise (-1 labels) should be drop
		:param centres_as_geometry: If true, centres of clusters are assigned as geometry
		:return: a spatially aggregated TrajectoriesFrame
		"""
...
class LayerAggregator():
	...
	def __init__(self, layer, kwargs):
		"""
		Class initalisation.
		:param layer: external, spatial layer which will be used for aggregation
		"""
	...
	def aggregate(self, trajectories_frame, dropoverwirte=True):
		...
		:param trajectories_frame: TrajectoriesFrame object class
		:param dropoverwirte: If true, points that were not assigned to any locationare removed.
		:return:
		"""
